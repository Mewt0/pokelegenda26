...
if (isset($_GET['go']) && isset($legacyGameRoutes[$_GET['go']])) {
  $newRoute = $legacyGameRoutes[$_GET['go']];
  if (function_exists('http_response_code')) http_response_code(410);
  header('Content-Type: text/html; charset=UTF-8');
  echo '<!doctype html><meta charset="utf-8"><title>Legacy route removed</title><h1>Legacy game route removed</h1><p><code>game.php?go=' . htmlspecialchars($_GET['go'], ENT_QUOTES, 'UTF-8') . '</code> is no longer an execution entrypoint.</p><p>Use <a href="' . htmlspecialchars($newRoute, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($newRoute, ENT_QUOTES, 'UTF-8') . '</a>.</p>';
  exit;
}
