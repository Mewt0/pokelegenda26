...
if(!empty($_POST['code']) && !empty($_SESSION['klikercode'])){
    if($_POST['code'] == $_SESSION['klikercode']){
      ...
      die("<script>
            ...
            parent._location.document.getElementById('errorCapth').innerHTML='Неверный код, повторите!';
          </script>");
    }
}
...
/** Смена покемона через пост *КОНЕЦ* **/
  ...
  if(poke_info($pokeOne,'hp_my') > 0){ 
    ...
    if(!empty($itemUser['item_id'])){
      if(provitems($itemUser['item_id'],1) && infoItems($itemUser['item_id'],'battleuse') == 1){
          if($itemUser['item_id'] == 3 && poke_info($pokeTwo,'poimka') == 0) { print "<script>alert('Этого покемона ловить нельзя!');</script>"; unset($_POST['itemgo']);} else {
            $po_p = "998";
...
$eventsText = '<span style="color:#fff;font-size:15px;font-weight:bold;cursor:pointer;" onClick="eventhtml();" title="Увеличеные рейты">Присутствуют увеличения  >></span>';
$eventsMess = false;
if($dropsEvent > 1) $eventsMess .= '<b style="color:#f754e1;">Дроп:</b> +<b>'.(($dropsEvent-1)*100).'%</b>.<br>';
if($expmyEvent > 1) $eventsMess .= '<b style="color:#7df9ff;">Опыт:</b> +<b>'.(($expmyEvent-1)*100).'%</b>.<br>';
if($maneyEvent > 1) $eventsMess .= '<b style="color:#ff7518;">Монеты:</b> +<b>'.(($maneyEvent-1)*100).'%</b>.<br>';
if($happyEvent > 1) $eventsMess .= '<b style="color:gold;">Счастье:</b> +<b>'.(($happyEvent-1)*100).'%</b>.<br>';
if($energEvent > 1) $eventsMess .= '<b style="color:#66ff00;">Энергия:</b> +<b>'.(($energEvent-1)*100).'%</b>.<br>';
...
if($hp_my > 0){
  if($hp_no > 0){
      ...
      ?>
        <tr>
          <td colspan=2 align="center">
            <INPUT type="button" title="Сдаться" value="Сдаться" onclick="window.location.href='/game.php?go=fight_pve&udar=sur';" style="width:120px;" >
          </td>
        </tr>
      </table>
    <?php
    ...
    /** Дроп, опыт, квесты, счастье **/
    if($hp_my > 0 && $pve_id > 0){
    print '<table width="60%"><tr><td><td><div id="atc" style="padding:10px;width:100%;text-align:left;font-weight:bold;">
          Выбито: Монет: x'.$maney.$dropMess.'.
          <br>
          Получено: Опыта: x'.$exp.', Энергии: x'.$enerone.', Счастья: x'.$happyone.'.
          </div></tr></table>';
    ...
    /** Дроп, опыт, квесты, счастье  *КОНЕЦ*  **/
 }
}else{
$my_pokes_yes = "";
    if($hp_one_user_poke){
    ...
    foreach($hp_one_user_poke as $smena_pok_id){
      ...
      $out =  '<INPUT type="button" title="Сдаться" value="Сдаться" onclick="location.href=\'/game.php?go=fight_pve&udar=sur\';" style="width:120px;" >';
    }else{
   ...
   if($tipPoke == 478 OR $tipPoke == 94 OR $tipPoke == 248 OR $tipPoke == 645 OR $tipPoke == 571 OR $tipPoke == 487) {
    ...
    <center><b>
    <a href='/game.php?go=fight_pve&code_new=1' target=\"_chat_two\" style='color:#afeeee; font-size: 15px;'>Обновить картинку</a><br>
    Введите, пожалуйста, цифры и буквы с картинки:<br>
    ...
    <input type='submit' value='ОК' name='OK' style='font-weight:bold; font-size: 15px;  color: #000; border:2px solid #000;'>
    </form>
    </center></div>";
}
...
<center>
  ...
  <div style="width:100%; text-align: left; height:14; border: 2px solid black; border-radius: 4px;">
    <div style='width:<?php echo $h1 ?>%;background:<?php echo $color1 ?>height:14;  border-radius: 4px;' title="Количество здоровья покемона">
      <font style='font:11px Tahoma; color: #1C1C1C;'>
