...
function colorsUsers($group){
    ...
    elseif ($group == 3)  $color_gr = "#000000"; //Мoдераторы
...
function rang_group($id,$group,$rang,$color=false) {
  ...
  elseif ($group == 5) $returnRang = "<span style=\"color:".$colors."\"><b>Гим-лидер</span>, ".$rang."</b></span>";
...
function color_group_users($id, $tipes = false){
  $inF = first('SELECT login,groups,online FROM users WHERE id=%d',$id);
  $login = $inF['login'];
  if($login){          
    $group = $inF['groups'];   
    $on    = $inF['online'];
...
function sistemChat($txt){
   return '<b><tt style=\"color:gold;\">['.date('H:i:s',time()).']</tt> <a href=\'javascript://\' onclick=\'parent.privat(\"Pokelegenda\")\'><font color=\"black\"> PR </font></a><a href=\'javascript://\' onclick=\'parent.to_tr(\"Pokelegenda\")\' style=\"color:'.colorsUsers(1).'\">Pokelegenda</a>: '.$txt.'</b><br>';
}
...
function moderations($login,$minut,$pricina){
  ...
  query('UPDATE usersunictable SET coolday=coolday-1 WHERE id=%d AND coolday >= 1 AND coolday < 30',$a['id']);
  text_chats($text,1);
  log_moder($text);     
...
function my_stat_go($id,$tip,$levels){
  hp_updates_pokemon($id);
  $p = first('SELECT evcount,exp,hp_max,users,lvl,id,basenum,har,hp_iv,hp_ev,atk_iv,atk_ev,def,def_ev,def_iv,satk_iv,satk_ev,sdef_ev,sdef_iv,speed_ev,speed_iv FROM pok_user WHERE users=%d AND id=%d',$_SESSION['id'],$id);
...
function itemsPokemon($id){
  $a = first('SELECT id_items FROM items_poke WHERE id_poke=%d',$id);
    if(!$a) $images_pok = '<img src=img/blank.gif width=24 height=24 border=0>';
      else
        $images_pok = '<img src=img/items/'.$item_pokes['id_items'].'.png width=24 height=24 border=0>';
 return $images_pok;
}
...
function updateQustPoke($id,$tip){
