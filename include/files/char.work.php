...
(function(){
  ...
  var url = 'game.php?go=".$route."&_=' + (new Date()).getTime();
  try { if(p && p._location && p._location.location){ p._location.location.href = url; return; } } catch(e) {}
  try { if(p && p.frames && p.frames['_location']){ p.frames['_location'].location.href = url; return; } } catch(e) {}
  try { if(p && p.loc){ p.loc('".$route."'); return; } } catch(e) {}
...
(function(){
  ...
  var locUrl = 'game.php?go=char&_=' + stamp;
  var usersUrl = '/gameusers&_=' + stamp;
  if(p && p.mess_chat){
    p.mess_chat('<b><font color=#000>->".$title."</font></b><br>');
  }
  var changed = false;
  try { if(p && p._location && p._location.location){ p._location.location.href = locUrl; changed = true; } } catch(e) {}
  try { if(!changed && p && p.frames && p.frames['_location']){ p.frames['_location'].location.href = locUrl; changed = true; } } catch(e) {}
  try { if(p && p._usersonline && p._usersonline.location){ p._usersonline.location.href = usersUrl; } } catch(e) {}
  try { if(p && p.frames && p.frames['_usersonline']){ p.frames['_usersonline'].location.href = usersUrl; } } catch(e) {}
  if(!changed){
