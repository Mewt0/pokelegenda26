...
if (isset($_POST['add']) && isset($_POST['itID']) && $_POST['itID'] > 0) {
    if ($myrow['pve'] > 0 || $myrow['pvp'] > 0 || $myrow['trade'] > 0) {
        $_SESSION['TEXT_ITEMS_ERROR'] = "Прежде чем совершить какое-либо действие с инвентарем, Вам необходимо закончить бой";
    ...
    if ($tip == "use") {
        ...
        if ($invent['timers'] != "not" && $invent['timers'] > time()) { $_SESSION['TEXT_ITEMS_ERROR'] = "Сейчас вы не можете использовать этот предмет"; die(no_href_item()); }
        if ($invent['dattimer'] != "not" && $invent['dattimer'] <= time()) { $_SESSION['TEXT_ITEMS_ERROR'] = "У предмета истек срок годности"; die(no_href_item()); }
    ...
    elseif ($tip == "drop") {
        ...
        if ($issetInv['delet'] != 0) { $_SESSION['TEXT_ITEMS_ERROR'] = "Нельзя удалить этот предмет"; die(no_href_item()); }

        minus_item($cools, $issetInv['id'], $itemId);
        die(no_href_item());
    }
    elseif ($tip == "clan") {
        $_SESSION['TEXT_ITEMS_ERROR'] = "На данный момент эта функция не работает";
        die(no_href_item());
        ...
        if ($invent['user_id'] != $_SESSION['id'] || $cools > $invent['count']) die(no_href_item());
        if ($issetInv['torg'] != 0) { $_SESSION['TEXT_ITEMS_ERROR'] = "Нельзя передать/продать этот предмет"; die(no_href_item()); }
