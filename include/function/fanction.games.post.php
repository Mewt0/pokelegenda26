    ...
    if(isset($_GET['sendZap']) && ((int)$_GET['sendZapStr']>0)){
      ...
      $sms = select('SELECT * FROM sends WHERE users=%d ORDER BY id DESC LIMIT %d,%d',$uses,$itemsinpage->get('Start'),$itemsinpage->get('Limit'));
        ...
        foreach($sms as $smsenter){ 
          if($smsenter['active'] == 1) $smsStatus = "Не прочитано"; else $smsStatus = "Прочитано";;
          print '                                                       
           <div id="send-'.$smsenter['id'].'" class="pane">
    	     <h3 onclick="effectSlow(\'#pid\','.$smsenter['id'].')">Состояние: <span style="color:#fadadd" class="sendsStatus">'.$smsStatus.'</span>, От: '.color_group_users($smsenter['inputusers']).', Тема: <span style="color:#fadadd">'.$smsenter['tema'].'.</span></h3>
    	     <div id="pid-'.$smsenter['id'].'" class="pid">
           '.$smsenter['text'].'
           <div class="otvets">
           <br>
           <hr style="background: #99958c;">
           Тема: <input id="sendSubj" value="Re..." maxlength="20"><br>
           Ответ:                                                   
                   ...
                   <br />
                   <center>
                   <input class="subSend" type="submit"  title="Отправить" value="Отправить"  onclick="sendGo('.$smsenter['id'].')">
                   </center>
           </div>
           </div>
    	     <img src="/css/img/btn-delete.png" style = "border:0px;" alt="Удалить" title="Удалить" class="delete"  onclick="dels('.$smsenter['id'].','.$st2.')">
           <img src="/css/img/crayon.png" style = "border:0px; width: 18px; height: 18px;" alt="Ответить" title="Ответить" id="btOtv" class="btOtv"  onclick="otvet('.$smsenter['id'].')">
           </div>                                                                                                                                                       
          ';
        }
        }               
