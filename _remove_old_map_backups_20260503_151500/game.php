...
<script src="/script/poke_spa.js?v=1"></script>
      ...
      document.addEventListener('click', function(e){
        ...
        if (/\/game\.php\?[^#]*\bgo=charWork\b/i.test(href)){
          ...
          ajaxGET(url, function(r){
            if (r && r.ok && r.redirect){
              ...
              window.location.replace('/game.php?go=map');
            }
          });
          return;
        }
        ...
        if (/\/game\.php\?[^#]*\bgo=map\b/i.test(href) && (/\bgets=/.test(href) || /\bnapadenie=/.test(href))){
          ...
          ajaxGET(url2, function(r){
            ...
            else { window.location.replace('/game.php?go=map'); }
          });
          return;
        }
    ...
    (function () {
      ...
      function connect() {
        ...
        es.addEventListener('challenge', function(e){
          ...
          // Обновляем карту
          fetch('/game.php?go=map&ajax=1').then(function(r){return r.text();}).then(function(h){ var el=document.getElementById('map'); if(el) el.innerHTML=h; });
  ...
  elseif ($go == "map") {
    require_once('include/files/map.world.php');
  }
  elseif ($go == "char") {
