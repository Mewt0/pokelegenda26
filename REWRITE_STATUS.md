...
Обновлено 2026-05-08: из `pokemon_moves_by_gen.xlsx` импортированы недостающие атаки по проверке имени, без изменения уже существующих атак. В живой `attac_power` добавлено 375 новых строк `atac_id=1000..1374`; общее количество атак стало 937. Описания и эффекты новых атак записаны на русском в UTF-8, миграция `2026_05_08_000004_import_missing_attacks.sql` идемпотентна и при повторном запуске не создает дубли.
