...
Обновлено 2026-05-27: Phase 2 PvP + Primal/Mega/weather stage подтверждён дополнительными smoke-прогонами. После основного PvP `Tacos -> NIGA` (`126/126`) выполнен обратный PvP `NIGA -> Tacos` (`126/126`), чтобы Groudon был активным первым покемоном; battle_log подтвердил `[TRANSFORM] Groudon -> Primal Groudon by red_orb` и `[WEATHER] Desolate Land activated by Primal Groudon`. Затем Rayquaza временно сделан стартовым у Tacos, PvP smoke снова прошёл `126/126`, battle_log подтвердил `[TRANSFORM] Rayquaza -> Mega Rayquaza by dragon_ascent` и `[WEATHER] Delta Stream activated by Mega Rayquaza`; стартовый флаг возвращён Kyogre. Отдельные static catalog checks `10/10` подтвердили правила Blue Orb, Red Orb, Dragon Ascent и mapping strong weather. После всех прогонов Tacos/NIGA вне боя, `db_integrity_smoke` остаётся `P0=0`, `P1=0`, `WARN=4` (`items=38`, `pokemon=63`, `eggs=1`, `battle.active_unfinished=208`).

Обновлено 2026-05-27: Phase 2 PvE/PvP hardening закрыла критичный риск пересечения PvE и PvP по одинаковому `battles.id`. Причина бага: legacy-схема без `AUTO_INCREMENT` могла создавать/оставлять строки `id=0` и положительные дубли, из-за чего PvE catch иногда видел старый PvP-row и отвечал `Покеболы нельзя использовать в PvP-бою`. Добавлена и применена миграция `2026_05_27_000008_battle_id_sequence.sql`: `battle_id_sequence` закрепляет общий allocator battle-id. `WildEncounterService`, `BattleRepository` и `BossRepository` теперь выделяют уникальные положительные id через sequence + `GET_LOCK`, не делают DDL внутри активной транзакции, не создают PvE rows с `id=0`, а активный бой ищется строго по `users.pve/users.pvp` и `battles.batl_tip`. `finishBattle()` ограничен PvE-row текущего игрока, чтобы дубль id не закрывал чужой/старый PvP. `IntegrityRepository` получил проверки `battle.zero_id_rows`, `battle.duplicate_positive_ids` и `users.stale_active_battle`; safe-fix удалил legacy zero-id battle rows и stale battle flags, один активный тестовый дубль был очищен вручную вместе с `pok_pve`. Проверено: миграции `63/63`, `pending=0`, `dirty=0`, `failed=0`; PHP lint изменённых battle/integrity файлов; PvE catch `57/57`, PvE finish/rewards/ack `58/58`, PvP Tacos/NIGA `126/126`, Commission `24/24`, background jobs `9/9`, Safe Storage `7/7`; integrity smoke `P0=0`, `P1=0`, `WARN=4` (`items=38`, `pokemon=63`, `eggs=1`, исторические unfinished battles). Browser QA `/game`: debug PvE открывает корректный бой, видны оба покемона, Primal Kyogre, Blue Orb, 4 атаки, weather и кнопки Fight/Pokémon/Items/Balls без критичных console errors.

Обновлено 2026-05-27: Phase 1.4 DB Integrity + антидюп получил отдельный проверочный слой. Добавлена и применена миграция `2026_05_27_000007_data_integrity_logs.sql`: `data_integrity_logs`, настройки `integrity.safe_fix_enabled` и `integrity.last_run_at`. Добавлен `IntegrityRepository` и CLI `tools/db_integrity_smoke.php [--fix-safe|--json]`. Проверки покрывают orphan/invalid inventory, held-item references, invalid pokemon/egg owners, market snapshot/reserve mismatch, permanent Primal/Mega corruption, finished active battle transformations, expired pending PvP requests, unfinished battles и Safe Storage pending/open rollback. Safe auto-fix намеренно ограничен: удаляет только `items_users.count <= 0`, orphan held rows, finished active transformations и expired PvP requests; orphan legacy owners остаются WARN для ручного решения, чтобы не потерять старые данные. Реальный `--fix-safe` убрал `1` мусорный inventory row. Текущий integrity smoke: `P0=0`, `P1=0`, `WARN=4`, `OK=13`: исторические orphan owners (`items=38`, `pokemon=63`, `eggs=1`) и `battle.active_unfinished=900` оставлены как предупреждения для следующих PvE/PvP этапов. Проверено: PHP lint `IntegrityRepository.php`, `tools/db_integrity_smoke.php`; миграции `62/62`, `pending=0`, `dirty=0`, `failed=0`.
...
|---|---:|---|
| Dev-сборка для активной разработки | 87% | Основные экраны/API живые, есть подготовленные QA-аккаунты, транзакционные smoke-скрипты, migration status, beta backup, Safe Storage, background jobs, integrity smoke и browser-regression по ключевым боевым/инвентарным сценариям. |
...
| PvE бой | 72% | Старт, атаки, статусы, погода, ловля, награды, finish/ack и Primal/Mega smoke закрыты; catch/finish повторно проверены после battle-id hardening, остаются редкие атаки, все предметы и долгие edge cases. |
| PvP бой | 73% | Invite/reject/timeout/accept/refresh/anti-double-click/history/items/surrender smoke прошёл `126/126` после battle-id hardening; до релиза нужны долгие реальные 6x6 бои и replay/audit. |
...
| DB integrity / антидюп | 65% | Integrity-smoke и safe-fix готовы, добавлены проверки zero/duplicate battle ids и stale battle flags, P0/P1 сейчас нет; остались ручные решения по orphan legacy owners и политика cleanup исторических unfinished battles. |
...
- `DB integrity / anti-dupe` - `PARTIAL_NEW`: smoke/fix-safe/logging есть; дальше нужны admin UI, whitelist legacy owner buckets и политика архивации старых battles.
...
| `safe_storage_entries`, `safe_operation_rollbacks`, `safe_storage_logs` | `PARTIAL_NEW` | Единый безопасный склад и rollback-журнал для item/pokemon/egg/currency операций: commission возвраты, gift/inventory ошибки, reward failures, breeding egg rollback-plan и battle reward rollback-plan. Покрыт `tools/safe_storage_smoke.php`; pending/failed проверяются `tools/beta_data_audit.php`. |
| `background_job_runs`, `background_job_logs` | `PARTIAL_NEW` | Лог запусков cron/background jobs: market expire, PvP timeout, temp items, event cleanup, transport/stuck scan и safe storage status. CLI `tools/background_jobs.php`, smoke `tools/background_jobs_smoke.php`. |
| `data_integrity_logs` | `PARTIAL_NEW` | Журнал integrity/anti-dupe проверок: orphan inventory/pokemon/eggs, market reserve/snapshot, Primal/Mega corruption, impossible battle state и safe-fix results. CLI `tools/db_integrity_smoke.php`. |
