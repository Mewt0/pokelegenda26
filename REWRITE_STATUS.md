...
| `/api/battle/pvp/*` | `PARTIAL_NEW` | Добавлены заявки на PvP, старт боя между тренерами, состояние/действия через общий боевой UI, polling входящих вызовов. |
...
- DB smoke: добавление ловушки, защита от дубля, чтение ловушек, Fire Punch -> burn, Leech Seed -> status 8, чтение inbox, PvP заявка/принятие/два хода в транзакции с rollback.
