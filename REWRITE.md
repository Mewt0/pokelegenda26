...
## Блок 2026-05-05: ловля и атаки после боя

Что было сломано:

- `pok_user.id` в живой БД не был `AUTO_INCREMENT`, а ловля дикого покемона вставляла строку без `id`.
- Пойманному покемону не создавалась строка `attac_my_poke`, поэтому после боя/ловли в меню можно было увидеть пустые слоты атак.
- Finished-состояние PvE отдавало `moves: []`, из-за чего UI боя рисовал четыре `Нет атаки` после завершения.

Что сделано:

- `BattleRepository::catchWildPokemon()` теперь явно выделяет следующий `pok_user.id`.
- Пойманному покемону сразу создаются стартовые атаки из `attac_poke`/`attac_power`.
- `PokemonRepository::setMove()` теперь умеет создать первую строку `attac_my_poke` с явным `id`.
- Finished-состояние PvE продолжает отдавать атаки игрока, а UI только блокирует действия.
- Добавлена миграция `database/migrations/2026_05_05_000002_harden_pokemon_capture_schema.sql` для ключей `pok_user`.

Проверки:

- PHP lint по изменённым боевым репозиториям и сервису.
- Реальный DB-сценарий ловли с последующей очисткой: создан временный `pok_user`, создан `attac_my_poke`, атаки появились.
