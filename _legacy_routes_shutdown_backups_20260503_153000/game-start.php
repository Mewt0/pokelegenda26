...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .chat-form button, .actionbar button {
      height: 38px;
      border: 1px solid var(--line);
      border-radius: 6px;
      background: #fff;
...
<body>
  <main class="world" data-csrf="<?= View::e($csrf) ?>">
    ...
    <nav class="actionbar">
      ...
      <button type="button">Обмен</button>
      <button type="button">Разведение</button>
      <button type="button">Бои</button>
  ...
  <script>
    ...
    function render(payload) {
      ...
      document.getElementById('locationText').textContent = 'Локация #' + location.id + '. Описание будет перенесено из legacy-слоя отдельным шагом.';
      ...
      document.getElementById('usersTitle').textContent = location.title + ' (' + users.length + ')';
