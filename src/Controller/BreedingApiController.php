<?php
declare(strict_types=1);

namespace Pokemon8\Controller;

use Pokemon8\Http\Request;
use Pokemon8\Http\Response;
use Pokemon8\Repository\BreedingRepository;
use Pokemon8\Security\Csrf;
use Pokemon8\Security\Session;

final class BreedingApiController
{
    public function __construct(
        private Session $session,
        private Csrf $csrf,
        private BreedingRepository $breeding,
    ) {
    }

    public function state(Request $request): Response
    {
        $userId = $this->userId();
        if ($userId <= 0) {
            return $this->json(['ok' => false, 'error' => 'auth', 'message' => 'Нужно войти в игру.'], 401);
        }

        return $this->json($this->breeding->state($userId));
    }

    public function createRequest(Request $request): Response
    {
        $userId = $this->userId();
        if ($userId <= 0) {
            return $this->json(['ok' => false, 'error' => 'auth', 'message' => 'Нужно войти в игру.'], 401);
        }
        if (!$this->csrf->validate($request->input('_csrf'))) {
            return $this->json(['ok' => false, 'error' => 'csrf', 'message' => 'Сессия устарела. Обновите страницу.'], 419);
        }

        return $this->json($this->breeding->createRequest(
            $userId,
            (string) $request->input('target', ''),
            (int) $request->input('pokemon_id', '0')
        ));
    }

    public function respond(Request $request): Response
    {
        $userId = $this->userId();
        if ($userId <= 0) {
            return $this->json(['ok' => false, 'error' => 'auth', 'message' => 'Нужно войти в игру.'], 401);
        }
        if (!$this->csrf->validate($request->input('_csrf'))) {
            return $this->json(['ok' => false, 'error' => 'csrf', 'message' => 'Сессия устарела. Обновите страницу.'], 419);
        }

        return $this->json($this->breeding->respond(
            $userId,
            (int) $request->input('request_id', '0'),
            strtolower((string) $request->input('action', '')),
            (int) $request->input('pokemon_id', '0')
        ));
    }

    public function dittoEssence(Request $request): Response
    {
        $userId = $this->userId();
        if ($userId <= 0) {
            return $this->json(['ok' => false, 'error' => 'auth', 'message' => 'Нужно войти в игру.'], 401);
        }
        if (!$this->csrf->validate($request->input('_csrf'))) {
            return $this->json(['ok' => false, 'error' => 'csrf', 'message' => 'Сессия устарела. Обновите страницу.'], 419);
        }

        return $this->json($this->breeding->useDittoEssence(
            $userId,
            (int) $request->input('pokemon_id', '0')
        ));
    }

    private function userId(): int
    {
        return (int) $this->session->get('id', 0);
    }

    private function json(array $payload, int $status = 200): Response
    {
        return new Response(
            json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?: '{}',
            $status,
            ['Content-Type' => 'application/json; charset=UTF-8']
        );
    }
}
