...
final class LocationRepository
{
    ...
    public function findUserState(int $userId): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT id, login, buildmy, pvp, pve, trade FROM users WHERE id = :id AND activation = 1 LIMIT 1'
        );
    ...
    public function usersAtLocation(int $locationId): array
    {
        $stmt = $this->db->prepare(
            'SELECT id, login, online, onlinetime, pve_button, pvp, trade
               FROM users
              WHERE buildmy = :location AND activation = 1
        ...
        foreach ($stmt->fetchAll() as $row) {
            $users[] = [
                ...
                'online' => ((int) $row['online'] === 1) && (time() - (int) $row['onlinetime'] <= 300),
