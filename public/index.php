...
use Pokemon8\Controller\BugReportApiController;
...
use Pokemon8\Controller\TournamentApiController;
...
$tournamentRepository = new TournamentRepository($db, $rewardRepository, $locations);
$economyGuardRepository = new EconomyGuardRepository($db);
$bugReportRepository = new BugReportRepository($db);
...
$adminApi = new AdminApiController($session, $csrf, $adminRepository, $bossRepository, $battleReplayRepository, $economyGuardRepository, $bugReportRepository);
...
$gameModules = new GameModuleController($session, $csrf, $transportRepository, $messageRepository, $itemMarketRepository, $eventRepository, $eggRepository, $pokemonMarketRepository, $quests, $commissionMarketRepository, $tournamentRepository);
...
$tournamentApi = new TournamentApiController($session, $csrf, $tournamentRepository);
$bugReportApi = new BugReportApiController($session, $csrf, $bugReportRepository);
...
$router->post('/api/bug-reports', fn (Request $request) => $bugReportApi->create($request));
...
$router->get('/api/tournaments', fn (Request $request) => $tournamentApi->index($request));
$router->post('/api/tournaments/register', fn (Request $request) => $tournamentApi->register($request));
$router->post('/api/tournaments/cancel', fn (Request $request) => $tournamentApi->cancel($request));
$router->post('/api/tournaments/arena-enter', fn (Request $request) => $tournamentApi->enterArena($request));
$router->post('/api/tournaments/arena-leave', fn (Request $request) => $tournamentApi->leaveArena($request));
$router->post('/api/tournaments/claim-reward', fn (Request $request) => $tournamentApi->claimReward($request));
...
$router->get('/api/profile/settings', fn (Request $request) => $profilePage->settings($request));
$router->post('/api/profile/settings', fn (Request $request) => $profilePage->saveSettings($request));
...
$router->get('/api/admin/qa-seed-tools', fn (Request $request) => $adminApi->qaSeedTools($request));
$router->post('/api/admin/qa-seed-tools/run', fn (Request $request) => $adminApi->runQaSeedTool($request));
...
$router->get('/api/admin/bug-reports', fn (Request $request) => $adminApi->bugReports($request));
$router->post('/api/admin/bug-reports/status', fn (Request $request) => $adminApi->updateBugReportStatus($request));
