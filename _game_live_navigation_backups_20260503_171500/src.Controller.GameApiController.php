...
final class GameApiController
{
    ...
    public function move(Request $request): Response
    {
        $userId = (int) $this->session->get('id', 0);
        if ($userId <= 0) {
            return $this->json(['ok' => false, 'error' => 'auth', 'message' => 'РќСѓР¶РЅРѕ РІРѕР№С‚Рё РІ РёРіСЂСѓ.'], 401);
        }

        if (!$this->csrf->validate($request->input('_csrf'))) {
            return $this->json(['ok' => false, 'error' => 'csrf', 'message' => 'РЎРµСЃСЃРёСЏ СѓСЃС‚Р°СЂРµР»Р°. РћР±РЅРѕРІРё СЃС‚СЂР°РЅРёС†Сѓ.'], 419);
        }
