...
if (!empty($_GET['quest_npc']) && !empty($_GET['do']) && ($_GET['quest_npc'] == 1 || $_GET['quest_npc'] == 2 || $_GET['quest_npc'] == 3)) {
    ...
    if (!empty($_GET['npc']) && ($_GET['npc'] == 1 || $_GET['npc'] == 2 || $_GET['npc'] == 3)) {
        ...
        $move  = '<a href="/game.php?go=charWork&loc=3"  data-go="charWork" data-params="loc=3">Лаборатория</a> | ';
        $move .= '<a href="/game.php?go=charWork&loc=17" data-go="charWork" data-params="loc=17">Здание Администрации</a> | ';
        $move .= '<a href="/game.php?go=charWork&loc=81" data-go="charWork" data-params="loc=81">Стадион Огненных покемонов</a> | ';
        $move .= '<s>Зона тренировки</s> | <s>Северная Алабастия</s> | ';
        $move .= '<a href="/game.php?go=charWork&loc=4"  data-go="charWork" data-params="loc=4">Дорога 1</a>';
